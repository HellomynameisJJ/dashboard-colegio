...
@section('content')
<div class="max-w-7xl mx-auto space-y-8 p-4 sm:p-6 lg:p-8 transition-colors duration-300 print:p-0 print:space-y-4">

    <!-- Topbar: Botón Volver + Acciones + Toggle Tema + Usuario en Sesión -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-3xl shadow-xl transition-colors duration-300 print:hidden">
        ...
        <div class="flex items-center justify-between sm:justify-end gap-3 flex-wrap">
            <button onclick="window.print()" type="button" aria-label="Imprimir Informe" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 text-xs font-bold hover:bg-indigo-100 dark:hover:bg-indigo-500/20 active:scale-95 transition-all duration-200 cursor-pointer">
                <i class="fa-solid fa-print text-xs"></i>
                <span class="hidden sm:inline">Imprimir Informe</span>
            </button>
    ...
    @php
        ...
        // Extracción de etiquetas y datos para Chart.js
        $chartLabels = [];
        $chartScores = [];
        foreach($results as $cName => $comps) {
            foreach($comps as $item) {
                $compName = is_object($item->competency) ? ($item->competency->name ?? $cName) : $cName;
                $chartLabels[] = $compName;
                $chartScores[] = min(20.0, (float)($item->score ?? 0));
            }
        }
    @endphp
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 print:grid-cols-3">
        <div class="lg:col-span-2 relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-xl transition-colors duration-300 print:shadow-none print:border-slate-300">
            <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5">
                <div class="relative">
                    <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 border border-indigo-400/30 flex items-center justify-center text-white text-3xl font-black shadow-xl shadow-indigo-600/20 print:shadow-none">
                        {{ strtoupper(substr($student->name ?? 'A', 0, 1)) }}
                    </div>
                    <span class="absolute -bottom-1 -right-1 w-6 h-6 bg-indigo-600 border-2 border-white dark:border-slate-900 rounded-full flex items-center justify-center text-[10px] text-white shadow print:hidden">
                ...
                <div>
                    <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 text-xs font-semibold mb-2">
                        <i class="fa-solid fa-id-card"></i> Expediente del Estudiante {{ $selectedCourseFilter !== 'all' ? '— Curso Filtrado' : '— Visión General' }}
                    </div>
            ...
            <div class="relative z-10 pt-6 mt-6 border-t border-slate-200 dark:border-slate-800 grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div class="col-span-2 sm:col-span-1">
                    ...
                    <span class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-0.5 rounded-lg border border-emerald-200 dark:border-emerald-500/20 mt-0.5">
                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse print:hidden"></span> Al día
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex flex-col justify-between shadow-xl transition-colors duration-300 print:shadow-none print:border-slate-300">
            <div>
    ...
    <!-- Panel Gráfico Radar Uniforme -->
    @if(count($results) > 0)
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl transition-colors duration-300 print:shadow-none print:border-slate-300">
    ...
    <div class="space-y-6">
        ...
        <div id="coursesContainer" class="space-y-6">
            @forelse($results as $courseName => $competencies)
                <div class="course-card bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl transition-colors duration-300 hover:border-slate-300 dark:hover:border-slate-700 print:shadow-none print:border-slate-300" data-course="{{ strtolower($courseName) }}">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        @foreach($competencies as $res)
                            @php 
                                $score = min(20.0, (float)($res->score ?? 0)); 
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (ctx) {
            let rawLabels = {!! json_encode($chartLabels) !!};
            let dataScores = {!! json_encode($chartScores) !!};

            // ASEGURAR TRIÁNGULO EN GRÁFICO SI HAY MENOS DE 3 EYES
            if (rawLabels.length === 2) {
                rawLabels.push('Diseña y construye soluciones tech...');
                let avg = dataScores.reduce((a, b) => a + b, 0) / 2;
                dataScores.push(Math.min(20, Math.round(avg)));
            }
                ...
                options: {
                    ...
                    scales: {
                        r: {
                            ...
                            pointLabels: { color: colors.labels, font: { size: 10, weight: '600' } },
                            ticks: { color: colors.labels, backdropColor: 'transparent', stepSize: 4 },
                            min: 0,
                            max: 20
                        }
                    }
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function(e) {
                ...
                courseCards.forEach(card => {
                    ...
                    card.style.display = (courseName.includes(query) || matchInCompetency) ? '' : 'none';
                });
            });
        }
    });
